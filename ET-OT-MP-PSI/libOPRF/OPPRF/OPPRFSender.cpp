...
namespace osuCrypto
{
	...
	NTL::ZZ OPPRFSender::block_to_ZZ(const block& b) {
		uint8_t buf[16];
		std::memcpy(buf, &b, 16);
		std::reverse(buf, buf + 16);  
		return NTL::ZZFromBytes(buf, 16);
	}

	block OPPRFSender::ZZ_to_block(const NTL::ZZ& zz) {
		uint8_t buf[16] = {};  
		long n = NTL::NumBytes(zz);
		if (n > 16) {
			throw std::runtime_error("NTL::ZZ too large to fit into block (128-bit)");
		}
		NTL::BytesFromZZ(buf + (16 - n), zz, n);  
		std::reverse(buf, buf + 16);            
		block out;
	...
	void  OPPRFSender::getOPRFkeysSeperatedandTable(u64 IdxP, binSet& bins, const std::vector<Channel*>& chls, bool isOtherDirectionGetOPRF)
	{
		...
		{
			auto seed = mPrng.get<block>(); // same sender generates the same seed for different IdxPs
			thrds[tIdx] = std::thread([&, tIdx, seed]() {
				...
				std::vector<block> ncoInput(bins.mNcoInputBlkSize); // 4
...
#pragma region compute Send Bark-OPRF				
				...
				auto binStart = tIdx       * otCountSend / thrds.size(); //0
				auto binEnd = (tIdx + 1) * otCountSend / thrds.size(); //binsNum
				...
				for (u64 bIdx = binStart; bIdx < binEnd;)
				{
					...
					for (u64 stepIdx = 0; stepIdx < currentStepSize; ++bIdx, ++stepIdx)
					{
						...
						if (bin.mIdx.size() > 0)
						{
							...
							//std::cout << "s-" << bIdx << ", ";
							// std::cout<<"myidx = "<<myIdx<<", bin = " << bIdx<<" OPRFvalue: ";
...
#pragma endregion
	...
	void OPPRFSender::sendSSTableBased(u64 IdxP, binSet& bins, std::vector<block>& plaintexts, const std::vector<Channel*>& chls, NTL::ZZ p)
	{
		...
		for (u64 tIdx = 0; tIdx < thrds.size(); ++tIdx)
		{
			auto seed = mPrng.get<block>(); //same sender generates same seed for different IdxP
			thrds[tIdx] = std::thread([&, tIdx, seed]() {
						...
						for (u64 stepIdx = 0; stepIdx < currentStepSize; ++bIdx, ++stepIdx)
						{
							...
							if (bin.mIdx.size() > 0)
							{
								...
								for (u64 i = 0; i < bin.mIdx.size(); ++i)
								{
									...
									NTL::ZZ num1 = NTL::ZZFromBytes((u8 *)&bin.mValOPRF[IdxP][i], sizeof(block));
									NTL::ZZ num2 = NTL::ZZFromBytes((u8 *)&plaintexts[inputIdx], sizeof(block));
									NTL::ZZ result = (num1 + num2) % p;
									block encr;
									NTL::BytesFromZZ((u8 *)&encr, result, sizeof(block));

									// NTL::ZZ num1 = block_to_ZZ(bin.mValOPRF[IdxP][i]);
									// NTL::ZZ num2 = block_to_ZZ(plaintexts[inputIdx]);
 									// NTL::ZZ result = (num1 + num2) % p;
									// block encr = ZZ_to_block(result);


									// NTL::ZZ temp = uint128_to_ZZ(bin.mValOPRF[IdxP][i]) + uint128_to_ZZ(plaintexts[inputIdx]);
									// temp = temp % p;
									// block encr = ZZ_to_ui128(temp);
								...
								{
									if (std::find(bin.mBits[IdxP].mMaps.begin(), bin.mBits[IdxP].mMaps.end(), i) == bin.mBits[IdxP].mMaps.end())////mMaps中没找到序号i
									{
...
#pragma endregion
	...
	void OPPRFSender::sendSSTableBased(u64 IdxP, binSet& bins, std::vector<std::vector<block>>& plaintexts, const std::vector<Channel*>& chls, NTL::ZZ p)
	{
		u64 row = bins.mSimpleBins.mNumHashes[0] + bins.mSimpleBins.mNumHashes[1];
		if (plaintexts.size() != row)
		{
			throw std::runtime_error(LOCATION);
		}
						...
						for (u64 stepIdx = 0; stepIdx < currentStepSize; ++bIdx, ++stepIdx)
						{
							...
							if (bin.mIdx.size() > 0)
							{
								...
								for (u64 i = 0; i < bin.mIdx.size(); ++i)
								{
									...
									// block encr = bin.mValOPRF[IdxP][i] ^ plaintexts[hashIdx][inputIdx];

									NTL::ZZ num1 = NTL::ZZFromBytes((u8 *)&bin.mValOPRF[IdxP][i], sizeof(block));
									NTL::ZZ num2 = NTL::ZZFromBytes((u8 *)&plaintexts[hashIdx][inputIdx], sizeof(block));
									NTL::ZZ result = (num1 + num2) % p;
									block encr;
									NTL::BytesFromZZ((u8 *)&encr, result, sizeof(block));
									...
									// NTL::ZZ num1 = block_to_ZZ(bin.mValOPRF[IdxP][i]);
									// NTL::ZZ num2 = block_to_ZZ(plaintexts[hashIdx][inputIdx]);
 									// NTL::ZZ result = (num1 + num2) % p;
									// block encr = ZZ_to_block(result);
