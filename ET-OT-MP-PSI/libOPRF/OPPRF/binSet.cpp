                ...
                for (u64 i = startIdx; i < endIdx; i += 128)
                {
                    ...
                    for (u64 hashIdx = 0; hashIdx < ncoInputHasher.size(); ++hashIdx)
                    {
                        ncoInputHasher[hashIdx].ecbEncBlocks(
							inputs.data() + i,
                            currentStepSize,
							mNcoInputBuff[hashIdx].data() + i); //hashing result:4*setsize
                    }
                    ...
                    {
						tempIdxBuff[j] = i + j; //element index:0~127,127~255,...,setsize-1
					...
					mSimpleBins.insertBatch(tempIdxBuff, hashes);//index,hash_result
