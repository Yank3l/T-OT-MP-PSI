...
namespace osuCrypto
{
	...
	NTL::ZZ OPPRFReceiver::block_to_ZZ(const block& b) {
		uint8_t buf[16];
		std::memcpy(buf, &b, 16);
	...
	block OPPRFReceiver::ZZ_to_block(const NTL::ZZ& zz) {
		uint8_t buf[16] = {};  
...
#pragma endregion
	...
	void OPPRFReceiver::recvSSTableBased(u64 IdxP, binSet& bins, std::vector<block>& plaintexts, const std::vector<Channel*>& chls, NTL::ZZ p)
	{
					...
					for (u64 bIdx = binStart; bIdx < binEnd;)
					{
						...
						for (u64 stepIdx = 0; stepIdx < curStepSize; ++bIdx, ++stepIdx)
						{
							...
							if (!bin.isEmpty())
							{
								...
								block mask_my, mask_their;
								std::memcpy(&mask_my, &myMask, sizeof(block));
								std::memcpy(&mask_their, &theirMask, sizeof(block));

								NTL::ZZ num1 = NTL::ZZFromBytes((u8 *)&mask_their, sizeof(block));
								NTL::ZZ num2 = NTL::ZZFromBytes((u8 *)&mask_my, sizeof(block));

								NTL::ZZ result = (num1 - num2) % p;
								NTL::BytesFromZZ((u8 *)&plaintexts[inputIdx], result, sizeof(block));
