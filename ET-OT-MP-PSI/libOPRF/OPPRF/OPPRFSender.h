...
namespace osuCrypto
{
    ...
    public:
		...
		void sendSSTableBased(u64 IdxTheirParty, binSet& bins, std::vector<block>& plaintexts,  const std::vector<Channel*>& chls, NTL::ZZ p);
		...
		void sendSSTableBased(u64 IdxTheirParty, binSet& bins, std::vector<std::vector<block>>& plaintexts,  const std::vector<Channel*>& chls, NTL::ZZ p);
		
		NTL::ZZ block_to_ZZ(const block& b);
		block ZZ_to_block(const NTL::ZZ& zz);
    };
