...
#pragma endregion
	...
	void OPPRFSender::sendSSTableBased(u64 IdxP, binSet& bins, std::vector<block>& plaintexts, const std::vector<Channel*>& chls, const std::vector<u64> FourModulo, const u64 index)
	{
						...
						for (u64 stepIdx = 0; stepIdx < currentStepSize; ++bIdx, ++stepIdx)
						{
							...
							if (bin.mIdx.size() > 0)
							{
								...
								for (u64 i = 0; i < bin.mIdx.size(); ++i)
								{
									...
									NTL::ZZ p = NTL::to_ZZ(FourModulo[index]);
...
#pragma endregion
	...
	void OPPRFSender::sendSSTableBased(u64 IdxP, binSet& bins, std::vector<block>& plaintexts, std::vector<std::vector<u128>> OLE_result,const std::vector<Channel*>& chls, const std::vector<u64> FourModulo, const u64 index)
	{
						...
						for (u64 stepIdx = 0; stepIdx < currentStepSize; ++bIdx, ++stepIdx)
						{
							...
							if (bin.mIdx.size() > 0)
							{
								...
								for (u64 i = 0; i < bin.mIdx.size(); ++i)
								{
									...
									u128 update_value = OLE_result[i][bIdx]; //simple hash table 中元素对应的OLE结果(4个32bit级联)
									
									block share_block = plaintexts[inputIdx];

									u128 share_128 = block_to_u128(share_block);

									

									u128 num1 = (share_128 & 0xFFFFFFFF) % FourModulo[index];
									u128 num2 = ((update_value >> ((3-index)*32)) & 0xFFFFFFFF) % FourModulo[index];
									u128 res = (num1 + num2) %  FourModulo[index];


									
									block program_value = u128_to_block(res);
									// block encr = bin.mValOPRF[IdxP][i] ^ program_value;

									NTL::ZZ num3 = NTL::ZZFromBytes((u8 *)&bin.mValOPRF[IdxP][i], sizeof(block));
									NTL::ZZ num4 = NTL::ZZFromBytes((u8 *)&program_value, sizeof(block));
									NTL::ZZ p = NTL::to_ZZ(FourModulo[index]);
									NTL::ZZ result = (num3 + num4) % p;
									block encr;
									NTL::BytesFromZZ((u8 *)&encr, result, sizeof(block));
...
#pragma endregion
	...
	void OPPRFSender::sendSSTableBased(u64 IdxP, binSet& bins, const std::vector<Channel*>& chls)
	{
					...
					for (u64 bIdx = binStart; bIdx < binEnd;)
					{
						...
						for (u64 stepIdx = 0; stepIdx < currentStepSize; ++bIdx, ++stepIdx)
						{
							...
							if (bin.mIdx.size() > 0)
							{
								...
								for (u64 i = 0; i < bin.mIdx.size(); ++i)
								{
									u128 randomNum = block_to_u128(generateRandomBlock());
									// u128 temp = i; //(random << 6 |i)
									u128 temp = (temp << 6) |i;
									block program_value = u128_to_block(temp);
									block encr = bin.mValOPRF[IdxP][i] ^ program_value;
