...
namespace lbcrypto {
    ...
    public:
        ...
        static constexpr ui32 logn = RingType::logn;  // 13
        static constexpr ui32 phim = (1 << logn);  // 2^13 = 8192
        ...
        constexpr EncodingContext(const value_type rou=0, const value_type phim_inv=0) {
            ...
            if (rou == 0)
                ftt_precompute(RootOfUnity(1<<(logn+1), modulus));//RootOfUnity(2^14,p)
            else
    ...
    struct DCRTEncodingContext {
        ...
        DCRTEncodingContext() {
            ...
            static_assert(
                std::is_same<typename InnerEncodingContextType::_, ptPoly>::value, 
