...
namespace lbcrypto {
    ...
    public:
        ...
        //getModulus for plaintext
        static constexpr value_type inline getPlaintextModulus(const ui32 modInd) {
            ASSERT_DEBUG(modInd < ParamsType::kMaxNbModuli);
            // std::cout << typeid(ParamsType).name() << std::endl;
            return  fast_four_limb_reduction_params::P_p[modInd];
        };
        ...
        static constexpr boost::multiprecision::uint256_t inline mod_mul_plaintext(const ui128 x, const ui128 y, const ui128 modInd, const bool opt) {
           const ui128 p = modInd;
            using ui512 = boost::multiprecision::uint512_t;
            using ui256 = boost::multiprecision::uint256_t;
            const ui256 pNewt("1095186299405284433905142782312543975888");
            ui256 res = ((ui256)x) * ((ui256)y);
            ui256 q = (((ui512)res * (ui512)pNewt) >> 256).convert_to<ui256>();
            ui256 r = (res - q * p);

            // std::cout<<"res = "<<res<<", q = "<<q<<std::endl;

            while(r>=p){
                r -= p;
            }
            // std::cout<<"r_end = "<<r<<std::endl;
            ui256 temp1 = x,temp2 = y;
            // std::cout<<"x = "<<temp1<<", y = "<<temp2 <<", res = "<<res<<", q = "<<q<<", r_end = "<<r<<std::endl;


            // ASSERT_DEBUG(r == mod_mul_slow(x, y, p));
            // if( r_end != mod_mul_slow(x, y, p))
            // {
