...
namespace lbcrypto {
    ...
    public:
        ...
        value_type qi_tilde_plaintext[2*numLimbs] __attribute__((aligned(32)));
        ui128 qi_star_qi_plaintext[2*numLimbs] __attribute__((aligned(32)));
        ...
        DCRT_Params() {
            ...
            {
                // std::cout << "DCRT_Params constructor called" << std::endl;

                std::fill(std::begin(qi_tilde), std::end(qi_tilde), 0);
                // std::cout << "qi_tilde_size: " << sizeof(qi_tilde) / sizeof(qi_tilde[0]) << std::endl;
                // std::cout<<"numExtLimbs: "<<numExtLimbs<<std::endl;
                ...
                for (ui32 i = 0; i < 4; i++) {
                    ...
                    for (ui32 j = 0; j < 4; j++) {
                        if (j != i)
                            //Mi = M/mi
                            // qi_star_qi = mod_mul_slow(qi_star_qi, IntRingType::getPlaintextModulus(j), IntRingType::getPlaintextModulus(i));
                            qi_star_qi *= IntRingType::getPlaintextModulus(j);
                    }
                    qi_star_qi_plaintext[i] = qi_star_qi;
                    qi_tilde_plaintext[i] = mod_inv((ui128)qi_star_qi, (ui128)IntRingType::getPlaintextModulus(i));
                    // std::cout<<"["<<i<<"]inv: "<<qi_tilde_plaintext[i]<<std::endl;
                }
    ...
    public:
        ...
        poly<RingType> compressDCRTPoly(const DCRTPoly<RingType, 4>& toCompress) const {
            ...
            // std::cout << demangle(typeid(greater_value_type).name()) << std::endl;
        ...
        template <typename RingType>
        std::vector<ui128> compressPlaintext(const DCRTPoly<RingType, 4>& toCompress, const ui128 p) const {
            std::vector<ui128> result;
            constexpr ui32 phim = RingType::phim;
            result.resize(phim);
            // // std::cout<<"use this"<<std::endl;
            for (ui32 i = 0; i < phim; i++) {
                uint256_t toMod = 0;
                for (ui32 j = 0; j < 4; j++) {
                    // toMod += IntRingType::mod_mul_plaintext(this->qi_tilde_plaintext[j], toCompress[j][i], j)*(1<<j); 
                    toMod = (uint256_t)toMod + ((uint256_t)toCompress[j][i]*(uint256_t)this->qi_tilde_plaintext[j]*(uint256_t)this->qi_star_qi_plaintext[j]) % p;
                    ...
                    result[i] = (toMod % p).convert_to<unsigned __int128>();
            //     result[i] = IntRingType::mod(toMod,0);
            }
            // return result;
        ...
        template <typename RingType>
        std::vector<ui128> compressPlaintext_opt(const DCRTPoly<RingType, 4>& toCompress, const ui128 p, const bool opt) const {
            std::vector<ui128> result;
            constexpr ui32 phim = RingType::phim;
            result.resize(phim);
            // // std::cout<<"use this"<<std::endl;
            for (ui32 i = 0; i < phim; i++) {
                ...
                for (ui32 j = 0; j < 4; j++) {
                    // toMod = (uint256_t)toMod + ((uint256_t)toCompress[j][i]*(uint256_t)this->qi_tilde_plaintext[j]*(uint256_t)this->qi_star_qi_plaintext[j]) % p;
                    uint256_t res1 = IntRingType::mod_mul_plaintext((ui128)toCompress[j][i],(ui128)this->qi_tilde_plaintext[j],p,opt);
                    uint256_t res2 = IntRingType::mod_mul_plaintext((ui128)res1,(ui128)this->qi_star_qi_plaintext[j],p,opt);
                    toMod += res2;
                    if (i == 10)
                    {
