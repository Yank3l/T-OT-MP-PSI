...
namespace lbcrypto {
    ...
    public:
        ...
        void send(osuCryptoNew::Channel& chl) const {
            chl.send(numBlocks);
        ...
        static VOLESenderInput<encoding_input_t> recv(osuCryptoNew::Channel& chl) {
            ui32 nB; chl.recv(nB);
    ...
    public:
        ...
        static VOLEReceiverOutput<encoding_input_t> receive(osuCryptoNew::Channel& chl) {
    ...
    public:
        ...
        using DCRTPoly = typename SchemeType::DCRTPoly_;
    ...
    public:
        ...
        static BOLEReceiverInput<encoding_input_t> receive(osuCryptoNew::Channel& chl) {
            ui32 n; chl.recv(n);
        ...
        void processPlainModulo(std::vector<std::vector<ui128>> ReInput, ui32 Module, ui32 shift){            
            for (ui32 i = 0; i < numBlocks; i++)
            {
                for (ui32 j = 0; j < encoding_input_t::phim; j++)
                {
                    x[i].vals[j] = (((ReInput[i][j]  >> (32 * (3-shift)))) & 0xFFFFFFFF)% Module; 
                    // x[i].vals[j] = ReInput[i][j] % Module;
                }
    ...
    template <typename encoding_input_t>
    class FourBOLEReceiverInputs {
    public:
        vector<ui32> FourModulo = {2820882433, 2183135233, 3997745153, 4294475777}; 

        
        std::vector<BOLEReceiverInput<encoding_input_t>> receiverInputs;

        
        FourBOLEReceiverInputs(ui128 numBlocks)
            : receiverInputs(4, BOLEReceiverInput<encoding_input_t>(numBlocks)) {} 

        
        FourBOLEReceiverInputs(const vector<encoding_input_t>& input1,
                            const vector<encoding_input_t>& input2,
                            const vector<encoding_input_t>& input3,
                            const vector<encoding_input_t>& input4)
            : receiverInputs{ BOLEReceiverInput<encoding_input_t>(input1),
                            BOLEReceiverInput<encoding_input_t>(input2),
                            BOLEReceiverInput<encoding_input_t>(input3),
                            BOLEReceiverInput<encoding_input_t>(input4) } {}

        
        void sendAll(osuCryptoNew::Channel& chl) const {
            for (const auto& receiverInput : receiverInputs) {
                receiverInput.send(chl);
            }
        }

       
        static FourBOLEReceiverInputs<encoding_input_t> receiveAll(osuCryptoNew::Channel& chl) {
            std::vector<BOLEReceiverInput<encoding_input_t>> receivedInputs;
            for (int i = 0; i < 4; ++i) {
                receivedInputs.push_back(BOLEReceiverInput<encoding_input_t>::receive(chl));
            }
            return FourBOLEReceiverInputs(receivedInputs[0].x, receivedInputs[1].x,
                                        receivedInputs[2].x, receivedInputs[3].x);
        }

        
        void processModule(std::vector<std::vector<ui128>> ReInput) {
            for (size_t i = 0; i < receiverInputs.size(); ++i) {
                receiverInputs[i].processPlainModulo(ReInput, FourModulo[i], i);
            }
        }
    };
    ...
    public:
        ...
        template <typename SchemeType>
        static BOLEReceiverOutput<typename SchemeType::encoding_input_t> online(
            const FourBOLEReceiverInputs<typename SchemeType::encoding_input_t>& input,
            const typename SchemeType::SecretKey& sk,
            const SchemeType& scheme, 
            osuCryptoNew::Channel& chl
        ) {
            ...
            using encoding_input_t = typename SchemeType::encoding_input_t;
            std::vector<BOLEReceiverOutput<encoding_input_t>> FourOutputs(4, BOLEReceiverOutput<encoding_input_t>(input.receiverInputs[0].numBlocks));
            for (ui32 i = 0; i < 1; i++)
            {
                FourOutputs[i] = online(input.receiverInputs[i], sk, scheme, chl);
            }
            
            return FourOutputs[0];
    ...
    template <typename SchemeType>
    typename SchemeType::DCRTPoly combine_plaintext(std::vector<BOLEReceiverOutput<typename SchemeType::encoding_context_t::encoding_input_t>> input){
