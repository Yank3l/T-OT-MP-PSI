...
namespace lbcrypto {
    ...
    public:
        ...
        typedef DCRTPoly<RingType, numLimbs> DCRTPoly_;
        ...
        typedef poly<RingType> poly_;

        /**Crypto Types **/

        // Keys
        typedef KeyPairDCRT<DCRTPoly_> KeyPair;
        typedef KeyPairDCRTSeeded<DCRTPoly_> KeyPairSeeded;
        typedef SecretKeyDCRT<DCRTPoly_> SecretKey;
        typedef PublicKeyDCRT<DCRTPoly_> PublicKey;
        typedef PublicKeyDCRTSeeded<DCRTPoly_> PublicKeySeeded;

        // Ciphertexts
        typedef DCRT_Ciphertext<DCRTPoly_> Ciphertext;
        typedef Single_Limb_Ciphertext<poly_> CompressedCiphertext;
        typedef DCRT_Seeded_Ciphertext<DCRTPoly_> SeededCiphertext;
        ...
        BFV_DCRT(double s_d = 3.2, ui32 w_sz = 2, ui32 pt_w_sz = 2) : std_dev(s_d), dgg(s_d) {
            ...
            static_assert(
                EncodingType::modulus == DCRTParamsType::plaintextModulus,    //EncodingType::modulus 是bole-class.p即明文模数
            ...
            for (ui32 i = 0; i < numLimbs; i++)             
                deltas.push_back((bigDelta % ubi(RingType::getModulus(i))).toUnsignedLong());//    (q/t % qi)
        };
        ...
        static constexpr inline DCRTPoly_ compute_shoup(const DCRTPoly_& input) {
            DCRTPoly_ result;
            for (ui32 i = 0; i < numLimbs; i++)
        ...
        static constexpr inline void compute_shoup(DCRTPoly_& result, const DCRTPoly_& input) {
            for (ui32 i = 0; i < numLimbs; i++)
                for (ui32 j = 0; j < phim; j++)
                    result.vals[i][j] = compute_shoup(input.vals[i][j], RingType::getModulus(i));
        };
        ...
        inline DCRTPoly_ packed_encode(const encoding_input_t& x) const {
            ptPoly pt_arr = encoding_context.packed_encode(x);
            DCRTPoly_ pt(pt_arr, true);
            return pt;
        }
        ...
        void inline sample_error(poly_& x) const {
            dgg.LoadVectorPoly<RingType, poly_::modInd>(x);
        }
        ...
        KeyPairSeeded KeyGenSeeded(const bool useSeed = false, const osuCryptoNew::block& seed = osuCryptoNew::ZeroBlock) const {
            ...
            else pk.seed = osuCryptoNew::sysRandomSeed();

            DCRTPoly_ a = DCRTPoly_(pk.seed);
            sample_error(pk.b);
            pk.b.ToEval(ntt_context);
        ...
        inline void Decrypt_NoDecode(plaintext_type discrim, ptPoly& result, const SecretKey& sk, const Ciphertext& ct) const {
            DCRTPoly_ toScale = mod_mul_shoup(ct.a, sk.s, sk.sShoup) + ct.b;
        ...
        inline void Decrypt_NoDecode(ui32 discrim, ptPoly& result, const SecretKey& sk, const CompressedCiphertext& ct) const {
            poly_ s_q1(sk.s.vals[0]);
            poly_ as_q1 = ct.a * s_q1;
            as_q1.ToCoeff(ntt_context.ntt_contexts[0]);

            poly_ toScaleDown = as_q1 + ct.b;
        ...
        ui32 NoiseMargin(const SecretKey& sk, const Ciphertext& ct) const {
            ...
            DCRTPoly_ toSub = mod_mul_shoup(ct.a, sk.s, sk.sShoup) + ct.b;
            toSub.ToCoeff(ntt_context);

            DCRTPoly_ toScale = packed_encode(message);
            toScale *= deltas;

            DCRTPoly_ toRecon = toSub - toScale;
        ...
        ui32 NoiseMargin(const SecretKey& sk, const CompressedCiphertext& ct) const {
            poly_ message = Decrypt(sk, ct);
            poly_ s_q1(sk.s.vals[0]);
            poly_ as_q1 = ct.a * s_q1;
            as_q1.ToCoeff(ntt_context.ntt_contexts[0]);

            poly_ toSub = as_q1 + ct.b;
            poly_ toScale(packed_encode(message).vals[0]);
            toScale *= deltaSmall;

            poly_ noise = toSub - toScale;
        ...
        Ciphertext Encrypt(const SecretKey& sk, const DCRTPoly_& pt) const {
            Ciphertext result;
            result.a.random(); 
        ...
        SeededCiphertext EncryptSeeded(const SecretKey& sk, const DCRTPoly_& pt) const {
            SeededCiphertext result;
            result.seed = osuCryptoNew::sysRandomSeed();
            // DCRTPoly a(result.seed);
            DCRTPoly_ a;
            // #pragma omp task default(shared)
            {
                a = DCRTPoly_(result.seed);
            }
            sample_error(result.b);
        ...
        Ciphertext Encrypt(const SecretKey& sk, const encoding_input_t& x) const {
            DCRTPoly_ pt = packed_encode(x);
            // std::cout<<"1  "<<std::endl;
            pt *= deltas; //delta = (q/t) % qi
            pt.ToEval(ntt_context); //NTT
        ...
        Ciphertext Encrypt(const PublicKey& pk, const DCRTPoly_& pt) const {
            DCRTPoly_ u, e, e_p;
        ...
        Ciphertext KDMEncrypt(const PublicKey& pk, const DCRTPoly_& pt) const {
        ...
        Ciphertext EvalSubPlain(const Ciphertext& ct, const DCRTPoly_& pt, const bool scale = true) const {
            DCRTPoly_ scaled(pt);
        ...
        Ciphertext EvalSubPlain(const DCRTPoly_& pt, const Ciphertext& ct, const bool scale = true) const {
            DCRTPoly_ scaled(pt);
            if (scale) scaled *= deltas;
            Ciphertext result;
            result.a = ct.a;
        ...
        inline void EvalSubPlainInPlace(Ciphertext& ct, const DCRTPoly_& pt) const {
            ct.b -= pt;
        };

        Ciphertext EvalAddPlain(const Ciphertext& ct, const DCRTPoly_& pt, const bool scale = true) const {
            DCRTPoly_ scaled(pt);
            if (scale) scaled *= deltas;
            Ciphertext result;
            result.a = ct.a;
        ...
        CompressedCiphertext EvalAddPlain(const CompressedCiphertext& ct, const poly_& x, const bool encode = true) const {
            CompressedCiphertext result;
            result.a = ct.a;
            result.b = ct.b + x;
            return result;
        ...
        // Ciphertext or CompressedCiphertext
        template <typename CiphertextType>
        inline void EvalAddPlainInPlace(CiphertextType& ct, const DCRTPoly_& pt) const {
            ct.b += pt;
        };

        DCRTPoly_ encodePlainMult(const ptPoly& x) const {
            ptPoly toEnc = encoding_context.packed_encode(x);
            DCRTPoly_ result(toEnc, true);  //DCRTPoly(const poly<RingType>& o, const bool noMod=false) : DCRTPoly(o.vals, noMod) {};
            result.ToEval(ntt_context);
            return result;
        };
 
        DCRTPoly_ encodePlainAdd(const ptPoly& x) const {
            DCRTPoly_ toScale = encodePlainMult(x);
            toScale *= deltas;
            return toScale;
        };

        encoding_input_t decodePlainAdd(const DCRTPoly_& input) const {
            DCRTPoly_ toScaleDown(input);
        ...
        encoding_input_t decodePlainAdd(const poly_& input) const {
            ptPoly scaledDown;
            for (ui32 i = 0; i < phim; i++) {
        ...
        Ciphertext EvalMultPlain(const Ciphertext& ct, const DCRTPoly_& pt) const {
            Ciphertext result;
            result.a = ct.a * pt;
            result.b = ct.b * pt;
            return result;
        ...
        inline void EvalMultPlainInPlace(Ciphertext& ct, const DCRTPoly_& pt) const {
            ct.a *= pt;
            ct.b *= pt;
        };
