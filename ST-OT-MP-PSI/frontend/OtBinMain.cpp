...
#pragma region setup
	...
	for (u64 i = 0; i < nParties; ++i)
	{
		if (i < myIdx)
		{
			u32 port = 1100 + i * 100 + myIdx;;//get the same port; i=1 & pIdx=2 =>port=102
...
#pragma endregion
		...
		{
			auto phase1 = std::chrono::duration_cast<std::chrono::milliseconds>(getSsDone - start).count(); //secret sharing + opprf
			auto phase2 = std::chrono::duration_cast<std::chrono::milliseconds>(phase2Done - getSsDone).count(); // update share(OLE)
